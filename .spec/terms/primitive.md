...
Eight types exist: term, invariant, rule, event, flow, contract, decision, feature. A primitive is identified by its (type, context, slug) triple. Shared primitives use the qualified ref `prefix:slug`. Contextual primitives use `context.prefix:slug`.
