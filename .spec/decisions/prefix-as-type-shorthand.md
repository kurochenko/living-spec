...
**Context:** CLI commands that accept a [[term:primitive-type]] as input (e.g., `lore add <type>`, `lore list --type <type>`) originally required the full type name (`feature`, `invariant`, etc.). Users already memorize the short prefixes from qualified IDs (`feat`, `inv`, etc.), so requiring the long form is friction.
