...
**Condition:** A directory can only have one [[term:spec-root]]. If `.spec/SPEC.md` already exists when `lore init` is run, the CLI prints `.spec/ already exists at {path}` and does nothing.
