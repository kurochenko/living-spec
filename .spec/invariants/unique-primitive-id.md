...
**Reference rule:** The short form `prefix:slug` resolves only shared primitives with no context. A primitive with a non-empty `context` must be referenced as `context.prefix:slug`.
