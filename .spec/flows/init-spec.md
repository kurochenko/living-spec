...
3. Create `.spec/` with subdirectories: `templates/`, `contexts/`, and one folder per primitive type (terms/, invariants/, rules/, events/, flows/, contracts/, decisions/, features/)
4. Write `.spec/SPEC.md` (the seed file with meta-model and LLM instructions)
5. Write `.spec/INDEX.md` (empty index)
6. Write `.spec/templates/{type}.md` for each of the eight types and `.spec/templates/context.md` for [[lore.term:context-overview]] files
